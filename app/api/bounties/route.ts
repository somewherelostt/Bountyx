...
const PAYMENT_REQUIREMENTS = {
  ...
  maxAmountRequired: "1", // 1 unit = 0.000001 USDC (absolute minimum for testnet)
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!TESTING_MODE) {
      ...
      if (!transferLog) {
        return NextResponse.json(
          { message: "No USDC transfer found" },
          { status: 400 }
        );
      }
      ...
      if (
        toAddress.toLowerCase() !== PAYMENT_REQUIREMENTS.payTo.toLowerCase()
      ) {
        return NextResponse.json(
